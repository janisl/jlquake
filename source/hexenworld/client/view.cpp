...
// don't count small mouse motion
	if (cl.nodrift)
	{
		if ( Q_fabs(cl.hw_frames[(cls.netchan.outgoingSequence-1)&UPDATE_MASK_HW].cmd.forwardmove) < (cl.v.hasted*cl_forwardspeed->value)-10 || lookspring->value == 0.0)
			cl.driftmove = 0;
		else
...
// roughly equal with different FOV
	...
	if ((cl.hw_frames[cls.netchan.incomingSequence&UPDATE_MASK_HW].playerstate[cl.playernum].drawflags & H2MLS_MASKIN) == H2MLS_POWERMODE)
...
//rjrcl.simangles[ROLL] = 0;	// FIXME @@@ 
	...
	view_frame = &cl.hw_frames[cls.netchan.incomingSequence & UPDATE_MASK_HW];
