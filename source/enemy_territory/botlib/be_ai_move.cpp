...
bot_movestate_t* botmovestates[MAX_CLIENTS_ET + 1];
...
// Changes Globals:		-
//========================================================================
void BotInitMoveState(int handle, bot_initmove_et_t* initmove)
{
	bot_movestate_t* ms;
...
*/
	...
	{
//BotImport_Print(PRT_MESSAGE, "BotTravel_Walk: NOT in range of reach (%i)\n", reach->areanum);
	}	//end if
...
//	vec3_t up = {0, 0, 1};
	...
	if (reach->start[2] > reach->end[2])
	{
		if (!(ms->moveflags & MFL_ONGROUND))
		{
			//BotImport_Print(PRT_MESSAGE, "not on ground\n");
			...
			//BotImport_Print(PRT_MESSAGE, "against ladder, descending\n");
		...
		{
			//BotImport_Print(PRT_MESSAGE, "moving towards ladder top\n");
			...
			{	//BotImport_Print(PRT_MESSAGE, "found top, moving towards ladder edge\n");
				VectorSubtract(reach->end, reach->start, dir);
				//make sure the horizontal movement is large anough
				dir[2] = 0;
				VectorNormalize(dir);
				...
				{
					//BotImport_Print(PRT_MESSAGE, "close to edge, backing in slowly..\n");
			...
			//BotImport_Print(PRT_MESSAGE, "speed = %.0f\n", speed);
			EA_Move(ms->client, dir, speed);
		}	//end else
	}
	else
