...
//		int usz = msg->uncompsize;
		...
		if (oldframe)
		{
			MSGET_WriteDeltaPlayerstate(msg, &oldframe->ps, &frame->ps);
		}
		else
		{
			MSGET_WriteDeltaPlayerstate(msg, NULL, &frame->ps);
		}
...
void SV_UpdateServerCommandsToClient(client_t* client, QMsg* msg)
{
	...
	for (i = client->reliableAcknowledge + 1; i <= client->reliableSequence; i++)
	{
		...
		msg->WriteLong(i);
		msg->WriteString(client->reliableCommands[i & (MAX_RELIABLE_COMMANDS_ET - 1)]);
	}
