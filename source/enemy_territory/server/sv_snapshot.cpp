...
static void SV_EmitPacketEntities(q3clientSnapshot_t* from, q3clientSnapshot_t* to, QMsg* msg)
{
	...
	while (newindex < to->num_entities || oldindex < from_num_entities)
	{
		...
		if (newnum == oldnum)
		{
			...
			MSGET_WriteDeltaEntity(msg, oldent, newent, qfalse);
			oldindex++;
			newindex++;
			continue;
		}
		...
		if (newnum < oldnum)
		{
			...
			MSGET_WriteDeltaEntity(msg, &sv.q3_svEntities[newnum].et_baseline, newent, qtrue);
			newindex++;
			continue;
		}
		...
		if (newnum > oldnum)
		{
			...
			MSGET_WriteDeltaEntity(msg, oldent, NULL, qtrue);
			oldindex++;
			continue;
		}
	}
...
void SV_UpdateServerCommandsToClient(client_t* client, QMsg* msg)
{
	...
	for (i = client->q3_reliableAcknowledge + 1; i <= client->q3_reliableSequence; i++)
	{
		...
		msg->WriteLong(i);
		msg->WriteString(client->q3_reliableCommands[i & (MAX_RELIABLE_COMMANDS_ET - 1)]);
	}
...
===============
*/
static void SV_AddEntToSnapshot(etsharedEntity_t* clientEnt, q3svEntity_t* svEnt, etsharedEntity_t* gEnt, snapshotEntityNumbers_t* eNums)
{
	...
	{
		if (!(qboolean)VM_Call(gvm, ETGAME_SNAPSHOT_CALLBACK, gEnt->s.number, clientEnt->s.number))
		{
			return;
		}
	}
	...
	for (e = 0; e < sv.q3_num_entities; e++)
	{
		...
		if (ent->r.svFlags & WOLFSVF_VISDUMMY)
		{
			...
			if (ment)
			{
				...
				master = SVET_SvEntityForGentity(ment);
...
//	q3clientSnapshot_t			*frame, *oldframe;
	...
	ps = SVET_GameClientNum(client - svs.clients);
	frame->et_ps = *ps;
	...
	{
		Com_Error(ERR_DROP, "SVET_SvEntityForGentity: bad gEnt");
	}
