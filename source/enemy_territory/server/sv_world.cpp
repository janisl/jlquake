...
===============
*/
void SV_UnlinkEntity(etsharedEntity_t* gEnt)
{
	q3svEntity_t* ent = SVET_SvEntityForGentity(gEnt);
...
void SV_LinkEntity(etsharedEntity_t* gEnt)
{
	worldSector_t* node;
	...
	ent = SVET_SvEntityForGentity(gEnt);
	...
	// encode the size into the etentityState_t for client prediction
	if (gEnt->r.bmodel)
	{
...
void SV_AreaEntities_r(worldSector_t* node, areaParms_t* ap)
{
	...
	for (check = node->entities; check; check = next)
	{
		...
		gcheck = SVET_GEntityForSvEntity(check);
...
void SV_ClipToEntity(q3trace_t* trace, const vec3_t start, const vec3_t mins, const vec3_t maxs, const vec3_t end, int entityNum, int contentmask, int capsule)
{
	...
	touch = SVET_GentityNum(entityNum);
