...
static void CM_TraceThroughLeaf( traceWork_t *tw, cLeaf_t *leaf ) {
	...
	for ( k = 0 ; k < leaf->numLeafBrushes ; k++ ) {
		...
		brush = &cm46->brushes[brushnum];
		if ( brush->checkcount == cm46->checkcount ) {
			continue;   // already checked this brush in another leaf
		}
		brush->checkcount = cm46->checkcount;
...
static void CM_Trace( q3trace_t *results, const vec3_t start, const vec3_t end,
	...
	cmod = cm46->ClipHandleToModel( model );
