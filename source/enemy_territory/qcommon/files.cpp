...
=================
*/
qboolean FS_PakIsPure( pack3_t *pack ) {
	int i;
...
int FS_FOpenFileRead( const char *filename, fileHandle_t *file, qboolean uniqueFILE ) {
	...
	if ( file == NULL ) {
		...
		for ( search = fs_searchpaths ; search ; search = search->next ) {
			//
			if ( search->pack3 ) {
				hash = FS_HashFileName( filename, search->pack3->hashSize );
			}
			// is the element a pak file?
			if ( search->pack3 && search->pack3->hashTable[hash] ) {
...
//#endif
			...
			if ( fs_restrict->integer || fs_numServerPaks ) {
				...
				if ( String::ICmp( filename + l - 4, ".cfg" )       // for config files
					 ...
					 && String::ICmp( filename, "qagame_mac" )
#endif
					 ) {
					continue;
				}
			...
			if ( fs_copyfiles->integer && !String::ICmp( dir->path, fs_cdpath->string ) ) {
				...
				FS_CopyFile( netpath, copypath );
			} else if ( fs_copyfiles->integer && fs_buildpath->string[0] && String::ICmp( dir->path, fs_buildpath->string ) ) {
				char    *copypath;
...
=================
*/
static pack3_t *FS_LoadZipFile( char *zipfile, const char *basename ) {
	...
	pack = (pack3_t*)Z_Malloc( sizeof( pack3_t ) + i * sizeof( fileInPack_t * ) );
	pack->hashSize = i;
...
const char *FS_ReferencedPakChecksums( void ) {
	...
	for ( search = fs_searchpaths ; search ; search = search->next ) {
		// is the element a pak file?
		if ( search->pack3 ) {
			if ( search->pack3->referenced || String::NICmp( search->pack3->pakGamename, BASEGAME, String::Length( BASEGAME ) ) ) {
				String::Cat( info, sizeof( info ), va( "%i ", search->pack3->checksum ) );
			}
		}
	}
...
const char *FS_LoadedPakChecksums( void ) {
	...
	for ( search = fs_searchpaths ; search ; search = search->next ) {
		...
		if ( String::Cmp( search->pack3->pakBasename,"pak0" ) ) {
			// this is a regular pk3
			String::Cat( info, sizeof( info ), va( "%i ", search->pack3->checksum ) );
		} else
		{
			// this is the light pk3
			String::Cat( info, sizeof( info ), va( "%s ", pak_checksums ) );
		}
	}
...
const char *FS_LoadedPakNames( void ) {
	...
	for ( search = fs_searchpaths ; search ; search = search->next ) {
		...
		{
			// light pk3
			String::Cat( info, sizeof( info ), pak_names );
		}
	}
...
const char *FS_LoadedPakPureChecksums( void ) {
	...
	for ( search = fs_searchpaths ; search ; search = search->next ) {
		...
		} else
		{
			...
			String::Cat( info, sizeof( info ), va( "%s ", FS_ShiftStr( pak_purechecksums[lookup_randomized[feed_index]], -13 - lookup_randomized[feed_index] ) ) );
			// use this if you are running on clear checksum strings instead of shifted ones
			//String::Cat( info, sizeof( info ), va("%s ", pak_purechecksums[lookup_randomized[feed_index]] ) );
		}
	}
