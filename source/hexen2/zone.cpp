...
{
	int extra;
	memblock_t* start, * rover, * newc, * base;
...
//
	...
	{	// there will be a free fragment after the allocated block
		newc = (memblock_t*)((byte*)base + size);
		newc->size = extra;
		newc->tag = 0;			// free block
		newc->prev = base;
		newc->id = ZONEID;
		newc->next = base->next;
		newc->next->prev = newc;
...
void Memory_Display_f(void)
{
	...
	{
		if (String::ICmp(Cmd_Argv(counter),"short") == 0)
		{
			all = false;
		}
		else if (String::ICmp(Cmd_Argv(counter),"save") == 0)
		{
			write_file = true;
		}
	}
...
const char* MemoryGroups[NUM_GROUPS + 1] =
{
	"texture",
	"light",
	"vis",
...
void Memory_Stats_f(void)
{
	...
	for (counter = 1; counter < NumItems; counter++)
	{
		if (String::ICmp(Cmd_Argv(counter),"save") == 0)
		{
			write_file = true;
		}
	}
	...
	Com_Memset(GroupSum,0,sizeof(GroupSum));
