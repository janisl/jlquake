...
void CL_ParseMuzzleFlash (void)
{
	...
	case MZ_MACHINEGUN:
		dl->color[0] = 1;dl->color[1] = 1;dl->color[2] = 0;
		QStr::Sprintf(soundname, sizeof(soundname), "weapons/machgf%ib.wav", (rand() % 5) + 1);
...
void CL_ParseMuzzleFlash2 (void) 
{
	...
	case MZ2_TANK_MACHINEGUN_19:
		...
		CL_ParticleEffect (origin, vec3_origin, 0, 40);
		CL_SmokeAndFlash(origin);
		QStr::Sprintf(soundname, sizeof(soundname), "tank/tnkatk2%c.wav", 'a' + rand() % 5);
...
#define	BEAMLENGTH			16
void CL_BfgParticles (refEntity_t *ent)
{
	int			i;
	cparticle_t	*p;
	float		angle;
...
*/
// RAFAEL
void CL_TrapParticles (refEntity_t *ent)
{
	vec3_t		move;
	vec3_t		vec;
	vec3_t		start, end;
