...
// weapons
	for (i = 0; i < 7; i++)
	{
		if (cl.qh_stats[QWSTAT_ITEMS] & (Q1IT_SHOTGUN << i))
		{
			...
			if (headsup)
			{
				...
				{
					Sbar_DrawSubPic((hudswap) ? 0 : (viddef.width - 24),-68 - (7 - i) * 16, sb_weapons[flashon][i],0,0,24,16);
				}
...
//			Sbar_DrawSubPic(3, -24, sb_ibar, 3, 0, 42,11);
			...
			if (num[0] != ' ')
			{
				Sbar_DrawCharacter((hudswap) ? 3 : (viddef.width - 39), -24 - (4 - i) * 11, 18 + num[0] - '0');
			}
			if (num[1] != ' ')
			{
				Sbar_DrawCharacter((hudswap) ? 11 : (viddef.width - 31), -24 - (4 - i) * 11, 18 + num[1] - '0');
			}
			if (num[2] != ' ')
			{
				Sbar_DrawCharacter((hudswap) ? 19 : (viddef.width - 23), -24 - (4 - i) * 11, 18 + num[2] - '0');
			}
		}
		else
		{
...
void Sbar_DrawFace(void)
{
	...
	if ((cl.qh_stats[QWSTAT_ITEMS] & (Q1IT_INVISIBILITY | Q1IT_INVULNERABILITY))
		== (Q1IT_INVISIBILITY | Q1IT_INVULNERABILITY))
	{
		Sbar_DrawPic(112, 0, sb_face_invis_invuln);
		return;
	}
	if (cl.qh_stats[QWSTAT_ITEMS] & Q1IT_QUAD)
	{
		Sbar_DrawPic(112, 0, sb_face_quad);
		return;
	}
	if (cl.qh_stats[QWSTAT_ITEMS] & Q1IT_INVISIBILITY)
	{
		Sbar_DrawPic(112, 0, sb_face_invis);
		return;
	}
	if (cl.qh_stats[QWSTAT_ITEMS] & Q1IT_INVULNERABILITY)
	{
...
// armor
	...
	else
	{
		...
		if (cl.qh_stats[QWSTAT_ITEMS] & Q1IT_ARMOR3)
		{
			Sbar_DrawPic(0, 0, sb_armor[2]);
		}
		else if (cl.qh_stats[QWSTAT_ITEMS] & Q1IT_ARMOR2)
		{
			Sbar_DrawPic(0, 0, sb_armor[1]);
		}
		else if (cl.qh_stats[QWSTAT_ITEMS] & Q1IT_ARMOR1)
		{
...
// ammo icon
	if (cl.qh_stats[QWSTAT_ITEMS] & Q1IT_SHELLS)
	{
		Sbar_DrawPic(224, 0, sb_ammo[0]);
	}
	else if (cl.qh_stats[QWSTAT_ITEMS] & Q1IT_NAILS)
	{
		Sbar_DrawPic(224, 0, sb_ammo[1]);
	}
	else if (cl.qh_stats[QWSTAT_ITEMS] & Q1IT_ROCKETS)
	{
		Sbar_DrawPic(224, 0, sb_ammo[2]);
	}
	else if (cl.qh_stats[QWSTAT_ITEMS] & Q1IT_CELLS)
	{
...
// draw the text
	...
	for (i = 0; i < scoreboardteams && y <= (int)viddef.height - 10; i++)
	{
		k = teamsort[i];
		tm = teams + k;
...
// draw the text
	...
	for (i = 0; i < scoreboardteams && y <= (int)viddef.height; i++)
	{
		k = teamsort[i];
		tm = teams + k;
