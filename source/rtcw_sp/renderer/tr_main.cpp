...
int R_CullLocalBox( vec3_t bounds[2] ) {
	...
	for ( i = 0 ; i < 8 ; i++ ) {
		...
		VectorMA( transformed[i], v[0], tr._or.axis[0], transformed[i] );
		VectorMA( transformed[i], v[1], tr._or.axis[1], transformed[i] );
		VectorMA( transformed[i], v[2], tr._or.axis[2], transformed[i] );
	}
...
void R_WorldToLocal( vec3_t world, vec3_t local ) {
	local[0] = DotProduct( world, tr._or.axis[0] );
	local[1] = DotProduct( world, tr._or.axis[1] );
	local[2] = DotProduct( world, tr._or.axis[2] );
}
...
void R_RotateForViewer( void ) {
	...
	VectorCopy( tr.viewParms._or.origin, origin );

	viewerMatrix[0] = tr.viewParms._or.axis[0][0];
	viewerMatrix[4] = tr.viewParms._or.axis[0][1];
	viewerMatrix[8] = tr.viewParms._or.axis[0][2];
	viewerMatrix[12] = -origin[0] * viewerMatrix[0] + - origin[1] * viewerMatrix[4] + - origin[2] * viewerMatrix[8];

	viewerMatrix[1] = tr.viewParms._or.axis[1][0];
	viewerMatrix[5] = tr.viewParms._or.axis[1][1];
	viewerMatrix[9] = tr.viewParms._or.axis[1][2];
	viewerMatrix[13] = -origin[0] * viewerMatrix[1] + - origin[1] * viewerMatrix[5] + - origin[2] * viewerMatrix[9];

	viewerMatrix[2] = tr.viewParms._or.axis[2][0];
	viewerMatrix[6] = tr.viewParms._or.axis[2][1];
	viewerMatrix[10] = tr.viewParms._or.axis[2][2];
...
static void SetFarClip( void ) {
	...
	for ( i = 0; i < 8; i++ )
	{
		...
		VectorSubtract( v, tr.viewParms._or.origin, vecTo );
...
void R_SetupFrustum( void ) {
	...
	VectorScale( tr.viewParms._or.axis[0], xs, tr.viewParms.frustum[0].normal );
	VectorMA( tr.viewParms.frustum[0].normal, xc, tr.viewParms._or.axis[1], tr.viewParms.frustum[0].normal );

	VectorScale( tr.viewParms._or.axis[0], xs, tr.viewParms.frustum[1].normal );
	VectorMA( tr.viewParms.frustum[1].normal, -xc, tr.viewParms._or.axis[1], tr.viewParms.frustum[1].normal );

	ang = tr.viewParms.fovY / 180 * M_PI * 0.5f;
	xs = sin( ang );
	xc = cos( ang );

	VectorScale( tr.viewParms._or.axis[0], xs, tr.viewParms.frustum[2].normal );
	VectorMA( tr.viewParms.frustum[2].normal, xc, tr.viewParms._or.axis[2], tr.viewParms.frustum[2].normal );

	VectorScale( tr.viewParms._or.axis[0], xs, tr.viewParms.frustum[3].normal );
	VectorMA( tr.viewParms.frustum[3].normal, -xc, tr.viewParms._or.axis[2], tr.viewParms.frustum[3].normal );

	for ( i = 0 ; i < 4 ; i++ ) {
		tr.viewParms.frustum[i].type = PLANE_NON_AXIAL;
		tr.viewParms.frustum[i].dist = DotProduct( tr.viewParms._or.origin, tr.viewParms.frustum[i].normal );
...
qboolean R_GetPortalOrientations( drawSurf_t *drawSurf, int entityNum,
	...
	if ( entityNum != ENTITYNUM_WORLD ) {
		...
		plane.dist = originalPlane.dist + DotProduct( plane.normal, tr._or.origin );

		// translate the original plane
		originalPlane.dist = originalPlane.dist + DotProduct( originalPlane.normal, tr._or.origin );
	} else {
		plane = originalPlane;
	}
