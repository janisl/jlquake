...
void RB_AddQuadStampFadingCornersExt( vec3_t origin, vec3_t left, vec3_t up, byte *color, float s1, float t1, float s2, float t2 ) {
	...
	VectorSubtract( vec3_origin, backEnd.viewParms._or.axis[0], normal );
...
static void RB_SurfaceSprite( void ) {
	...
	if ( backEnd.currentEntity->e.rotation == 0 ) {
		VectorScale( backEnd.viewParms._or.axis[1], radius, left );
		VectorScale( backEnd.viewParms._or.axis[2], radius, up );
	} else {
		float s, c;
		float ang;
		...
		VectorScale( backEnd.viewParms._or.axis[1], c * radius, left );
		VectorMA( left, -s * radius, backEnd.viewParms._or.axis[2], left );

		VectorScale( backEnd.viewParms._or.axis[2], c * radius, up );
		VectorMA( up, s * radius, backEnd.viewParms._or.axis[1], up );
	}
	if ( backEnd.viewParms.isMirror ) {
		VectorSubtract( vec3_origin, left, left );
	}
...
static float    LodErrorForVolume( vec3_t local, float radius ) {
	...
	d = DotProduct( world, backEnd.viewParms._or.axis[0] );
