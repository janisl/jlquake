...
==================
*/
void CL_DeltaEntity(QMsg* msg, wsclSnapshot_t* frame, int newnum, wsentityState_t* old,
	qboolean unchanged)
{
	...
	state = &cl.ws_parseEntities[cl.parseEntitiesNum & (MAX_PARSE_ENTITIES_Q3 - 1)];
...
==================
*/
void CL_ParsePacketEntities(QMsg* msg, wsclSnapshot_t* oldframe, wsclSnapshot_t* newframe)
{
	int newnum;
...
void CL_ParseGamestate(QMsg* msg)
{
	...
	while (1)
	{
		...
		if (cmd == q3svc_configstring)
		{
			...
			memcpy(cl.ws_gameState.stringData + cl.ws_gameState.dataCount, s, len + 1);
