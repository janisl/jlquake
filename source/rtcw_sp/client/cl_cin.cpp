...
typedef struct {
	...
	qboolean looping, holdAtEnd, alterGameState, shader, letterBox, sound;
	e_status status;
...
//----(SA)	modified to use specified fps for roq's
	...
	// we need to use CL_ScaledMilliseconds because of the smp mode calls from the renderer
	cinTable[currentHandle].tfps = ( ( ( CL_ScaledMilliseconds() - cinTable[currentHandle].startTime ) * cinTable[currentHandle].Cin->roqFPS ) / 1000 );
