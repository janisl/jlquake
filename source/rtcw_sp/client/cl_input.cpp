...
void IN_Notebook(void)
{
	if (cls.state == CA_ACTIVE && !clc.demoplaying)
	{
		...
		VM_Call(uivm, WSUI_SET_ACTIVE_MENU, WSUIMENU_NOTEBOOK);		// startup notebook
	}
}
...
void IN_Help(void)
{
	if (cls.state == CA_ACTIVE && !clc.demoplaying)
	{
		VM_Call(uivm, WSUI_SET_ACTIVE_MENU, WSUIMENU_HELP);			// startup help system
	}
}
...
void CL_MouseEvent(int dx, int dy, int time)
{
	...
	{
		VM_Call(uivm, WSUI_MOUSE_EVENT, dx, dy);
	}
