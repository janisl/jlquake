...
char **FS_ListFilteredFiles( const char *path, const char *extension, char *filter, int *numfiles ) {
	...
	for ( search = fs_searchpaths ; search ; search = search->next ) {
		...
		if ( search->pack3 ) {
			...
			if ( ( fs_restrict->integer || fs_numServerPaks ) && String::ICmp( extension, "svg" ) ) {
				continue;
			} else {
...
int FS_GetModList( char *listbuf, int bufsize ) {
	...
	*listbuf = 0;
	...
	if ( fs_cdpath->string && String::Length( fs_cdpath->string ) ) {
...
//----(SA)	SP mod
		...
		if ( !String::NCmp( sorted[i],"sp_",3 ) ) { //	sort sp first
	...
	for ( i = 0 ; i < numfiles ; i++ ) {
		...
		if ( String::NCmp( sorted[i],"mp_",3 ) ) { // (SA) SP mod -- exclude mp_*
