...
================
*/
clipHandle_t SV_ClipHandleForEntity(const wssharedEntity_t* ent)
{
	if (ent->r.bmodel)
	{
...
===============
*/
void SV_UnlinkEntity(wssharedEntity_t* gEnt)
{
...
void SV_LinkEntity(wssharedEntity_t* gEnt)
{
	worldSector_t* node;
	...
	ent = SVWS_SvEntityForGentity(gEnt);
...
void SV_AreaEntities_r(worldSector_t* node, areaParms_t* ap)
{
	...
	for (check = node->entities; check; check = next)
	{
		...
		gcheck = SVWS_GEntityForSvEntity(check);
...
{
	wssharedEntity_t* touch;
	clipHandle_t clipHandle;
	float* origin, * angles;

	touch = SVWS_GentityNum(entityNum);
...
void SV_ClipMoveToEntities(moveclip_t* clip)
{
	...
	for (i = 0; i < num; i++)
	{
		...
		touch = SVWS_GentityNum(touchlist[i]);
