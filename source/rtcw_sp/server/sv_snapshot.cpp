...
static void SV_EmitPacketEntities(q3clientSnapshot_t* from, q3clientSnapshot_t* to, QMsg* msg)
{
	...
	while (newindex < to->num_entities || oldindex < from_num_entities)
	{
		...
		if (newnum == oldnum)
		{
			...
			MSGWS_WriteDeltaEntity(msg, oldent, newent, qfalse);
			oldindex++;
			newindex++;
			continue;
		}
		...
		if (newnum > oldnum)
		{
			...
			MSGWS_WriteDeltaEntity(msg, oldent, NULL, qtrue);
			oldindex++;
			continue;
		}
	}
	...
	snapshotEntityNumbers_t* eNums, qboolean portal, qboolean localClient)
{
	...
	playerEnt = SVWS_GentityNum(frame->ws_ps.clientNum);

	for (e = 0; e < sv.q3_num_entities; e++)
	{
		ent = SVWS_GentityNum(e);
		...
		if ((playerEnt->s.eFlags & WSEF_VIEWING_CAMERA) && !portal)
		{
...
//				SV_AddEntitiesVisibleFromPoint( ent->s.origin2, frame, eNums, qtrue, oldframe, localClient );
		...
		if (ent->r.svFlags & WOLFSVF_VISDUMMY)
		{
			...
			//find master;
			ment = SVWS_GentityNum(ent->s.otherEntityNum);

			if (ment)
			{
				q3svEntity_t* master = 0;
				master = SVWS_SvEntityForGentity(ment);
...
notVisible:
		...
		if (svt3_gametype->integer == Q3GT_SINGLE_PLAYER && localClient)
		{
			if (ent->r.eventTime == svs.q3_time)
			{
				ent->s.eFlags |= WSEF_NODRAW;		// don't draw, just process event
				SV_AddEntToSnapshot(svEnt, ent, eNums);
			}
			else if (ent->s.eType == Q3ET_PLAYER)
			{
				// keep players around if they are alive and active (so sounds dont get messed up)
				if (!(ent->s.eFlags & WSEF_DEAD))
				{
					ent->s.eFlags |= WSEF_NODRAW;		// don't draw, just process events and sounds
					SV_AddEntToSnapshot(svEnt, ent, eNums);
				}
			}
		}
...
//	}
	...
	ps = SVWS_GameClientNum(client - svs.clients);
	frame->ws_ps = *ps;
	...
	{
		Com_Error(ERR_DROP, "SVWS_SvEntityForGentity: bad gEnt");
	}
...
//	SV_AddEntitiesVisibleFromPoint( org, frame, &entityNumbers, qfalse, oldframe, client->netchan.remoteAddress.type == NA_LOOPBACK );
	...
	{
		ent = SVWS_GentityNum(entityNumbers.snapshotEntities[i]);
		state = &svs.ws_snapshotEntities[svs.q3_nextSnapshotEntities % svs.q3_numSnapshotEntities];
		*state = ent->s;
