...
static void SV_MapRestart_f(void)
{
	...
	{
		if (sv_gametype->integer == Q3GT_SINGLE_PLAYER)		// (SA) no pause by default in sp
		{
			delay = 0;
		}
		else
	...
	for (i = 0; i < sv_maxclients->integer; i++)
	{
		...
		denied = (char*)VM_ExplicitArgPtr(gvm, VM_Call(gvm, WSGAME_CLIENT_CONNECT, i, qfalse, isBot));
		if (denied)
		{
		...
		SV_ClientEnterWorld(client, &client->ws_lastUsercmd);
	}
