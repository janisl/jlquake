...
void CM_TestInLeaf( traceWork_t *tw, cLeaf_t *leaf ) {
	...
	for (k=0 ; k<leaf->numLeafBrushes ; k++) {
		...
		if (b->checkcount == CMap->checkcount) {
			continue;	// already checked this brush in another leaf
		}
		b->checkcount = CMap->checkcount;
	...
	if ( !cm_noCurves->integer ) {
		for ( k = 0 ; k < leaf->numLeafSurfaces ; k++ ) {
			patch = CMap->surfaces[ CMap->leafsurfaces[ leaf->firstLeafSurface + k ] ];
			if ( !patch ) {
				continue;
			}
			if ( patch->checkcount == CMap->checkcount ) {
				continue;	// already checked this brush in another leaf
			}
			patch->checkcount = CMap->checkcount;
...
void CM_PositionTest( traceWork_t *tw ) {
	...
	for (i=0 ; i < ll.count ; i++) {
		CM_TestInLeaf( tw, &CMap->leafs[leafs[i]] );
		if ( tw->trace.allsolid ) {
			break;
		}
	}
...
void CM_TraceThroughTree( traceWork_t *tw, int num, float p1f, float p2f, vec3_t p1, vec3_t p2) {
	...
	if (num < 0) {
		CM_TraceThroughLeaf( tw, &CMap->leafs[-1-num] );
		return;
	}
