...
void CM46_FreeWinding(winding_t* w)
{
	...
	{
		common->FatalError("FreeWinding: freed a freed winding");
	}
...
winding_t* CM46_BaseWindingForPlane(vec3_t normal, vec_t dist)
{
	...
	if (x == -1)
	{
		common->Error("BaseWindingForPlane: no axis found");
	}
...
void CM46_ChopWindingInPlace(winding_t** inout, vec3_t normal, vec_t dist, vec_t epsilon)
{
	...
	{
		common->Error("ClipWinding: points exceeded estimate");
	}
	if (f->numpoints > MAX_POINTS_ON_WINDING)
	{
		common->Error("ClipWinding: MAX_POINTS_ON_WINDING");
	}
