...
void SV_AddServerCommand(client_t* client, const char* cmd)
{
	...
	if (client->reliableSequence - client->reliableAcknowledge == MAX_RELIABLE_COMMANDS_WM + 1)
	{
		Com_Printf("===== pending server commands =====\n");
		for (i = client->reliableAcknowledge + 1; i <= client->reliableSequence; i++)
		{
			Com_Printf("cmd %5d: %s\n", i, client->reliableCommands[i & (MAX_RELIABLE_COMMANDS_WM - 1)]);
		}
		Com_Printf("cmd %5d: %s\n", i, cmd);
		SV_DropClient(client, "Server command overflow");
		return;
	}
	index = client->reliableSequence & (MAX_RELIABLE_COMMANDS_WM - 1);
...
void SVC_Status(netadr_t from)
{
	...
	if (Cvar_VariableValue("g_gametype") == WMGT_SINGLE_PLAYER)
	{
		return;
	}
...
void SVC_Info(netadr_t from)
{
	...
	Info_SetValueForKey(infostring, "tourney", va("%i", sv_tourney->integer), MAX_INFO_STRING_Q3);					// NERVE - SMF
