...
void SV_SetConfigstring(int index, const char* val)
{
	...
	{
//		SVT3_SendServerCommand( NULL, "cs %i \"%s\"\n", index, val );
		...
		for (i = 0, client = svs.clients; i < sv_maxclients->integer; i++, client++)
		{
			...
			if (index == Q3CS_SERVERINFO && client->wm_gentity && (client->wm_gentity->r.svFlags & WOLFSVF_NOSERVERINFO))
			{
				continue;
			}

			// RF, don't send to bot/AI
			if (sv_gametype->integer == Q3GT_SINGLE_PLAYER && client->wm_gentity && (client->wm_gentity->r.svFlags & WMSVF_CASTAI))
			{
				continue;
			}
...
//	Cvar_Set( "nextmap", va("map %s", server) );
	...
	if (sv_gametype->integer == Q3GT_SINGLE_PLAYER || sv_gametype->integer >= WMGT_WOLF)
	{
