...
======================
*/
static void R_AddWorldSurface( mbrush46_surface_t *surf, shader_t *shader, int dlightBits ) {
	...
	R_AddDrawSurf( surf->data, shader, surf->fogIndex, dlightBits, 0, 0 );
}
...
//----(SA) modified
	...
	for ( i = 0 ; i < bmodel->numSurfaces ; i++ ) {
		...
		if ( ent->e.customShader ) {
			R_AddWorldSurface( bmodel->firstSurface + i, R_GetShaderByHandle( ent->e.customShader ), tr.currentEntity->dlightBits );
		} else {
			R_AddWorldSurface( bmodel->firstSurface + i, ( ( mbrush46_surface_t * )( bmodel->firstSurface + i ) )->shader, tr.currentEntity->dlightBits );
		}
	}
//----(SA) end
}
