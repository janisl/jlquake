...
static void CLQ2_DeltaEntity( QMsg& message, q2frame_t* frame, int newnum, q2entity_state_t* old, int bits ) {
	...
	if ( ent->serverframe != cl.q2_frame.serverframe - 1 ) {// wasn't in last update, so initialize some things
...
static void CLQ2_ParsePacketEntities( QMsg& message, q2frame_t* oldframe, q2frame_t* newframe ) {
	...
	while ( 1 ) {
		...
		if ( bits & Q2U_REMOVE ) {	// the entity present in oldframe is not in the current frame
			if ( cl_shownet->value == 3 ) {
...
//======
		...
		if ( renderfx_old & ( Q2RF_FRAMELERP | Q2RF_BEAM ) ) {	// step origin discretely, because the frames
...
//============
		...
		// calculate angles
		vec3_t angles;
		if ( effects & Q2EF_ROTATE ) {	// some bonus items auto-rotate
			angles[ 0 ] = 0;
			angles[ 1 ] = autorotate;
			angles[ 2 ] = 0;
		}
