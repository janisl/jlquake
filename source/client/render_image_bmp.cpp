...
void R_LoadBMP(const char* name, byte** pic, int* width, int* height)
{
	*pic = NULL;
	...
	{
		throw QDropException(va("LoadBMP: only uncompressed BMP files supported (%s)\n", name));
	}
	if (bmpHeader.bitsPerPixel < 8)
	{
		throw QDropException(va("LoadBMP: monochrome and 4-bit BMP files not supported (%s)\n", name));
	}
	...
	*pic = bmpRGBA;
	...
	for (int row = rows - 1; row >= 0; row--)
	{
		...
		for (int column = 0; column < columns; column++)
		{
			...
			default:
				throw QDropException(va("LoadBMP: illegal pixel_size '%d' in file '%s'\n", bmpHeader.bitsPerPixel, name));
				break;
			}
		}
	}
