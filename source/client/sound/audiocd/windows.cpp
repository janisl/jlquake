...
static void CDAudio_Eject() {
	...
	if ( dwReturn = mciSendCommand( wDeviceID, MCI_SET, MCI_SET_DOOR_OPEN, ( DWORD )NULL ) ) {
		common->DPrintf( "MCI_SET_DOOR_OPEN failed (%i)\n", dwReturn );
	}
}
...
static void CDAudio_CloseDoor() {
	...
	if ( dwReturn = mciSendCommand( wDeviceID, MCI_SET, MCI_SET_DOOR_CLOSED, ( DWORD )NULL ) ) {
		common->DPrintf( "MCI_SET_DOOR_CLOSED failed (%i)\n", dwReturn );
	}
}
...
static int CDAudio_GetAudioDiskInfo() {
	...
	if ( dwReturn ) {
		common->DPrintf( "CDAudio: drive ready test - get status failed\n" );
		return -1;
	}
	if ( !mciStatusParms.dwReturn ) {
		common->DPrintf( "CDAudio: drive not ready\n" );
		return -1;
	}
	...
	dwReturn = mciSendCommand( wDeviceID, MCI_STATUS, MCI_STATUS_ITEM | MCI_WAIT, ( DWORD )( LPVOID )&mciStatusParms );
	if ( dwReturn ) {
		common->DPrintf( "CDAudio: get tracks - status failed\n" );
		return -1;
	}
...
static void CDAudio_Play2( int track, qboolean looping ) {
	...
	if ( dwReturn ) {
		common->DPrintf( "MCI_STATUS failed (%i)\n", dwReturn );
		return;
	}
	...
	if ( dwReturn ) {
		common->DPrintf( "CDAudio: MCI_PLAY failed (%i)\n", dwReturn );
		return;
	}
...
void CDAudio_Stop() {
	...
	if ( dwReturn = mciSendCommand( wDeviceID, MCI_STOP, 0, ( DWORD )NULL ) ) {
		common->DPrintf( "MCI_STOP failed (%i)", dwReturn );
	}
...
void CDAudio_Pause() {
	...
	if ( dwReturn = mciSendCommand( wDeviceID, MCI_PAUSE, 0, ( DWORD )( LPVOID )&mciGenericParms ) ) {
		common->DPrintf( "MCI_PAUSE failed (%i)", dwReturn );
	}
...
static void CD_f() {
	...
	if ( Cmd_Argc() < 2 ) {
		...
		common->Printf( "on, off, reset, remap, \n" );
		common->Printf( "play, stop, loop, pause, resume\n" );
		common->Printf( "eject, close, info\n" );
		return;
	}
...
LONG CDAudio_MessageHandler( HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam ) {
	...
	case MCI_NOTIFY_FAILURE:
		common->DPrintf( "MCI_NOTIFY_FAILURE\n" );
		CDAudio_Stop();
		cdValid = false;
		break;

	default:
		common->DPrintf( "Unexpected MM_MCINOTIFY type (%i)\n", wParam );
		return 1;
	}
...
int CDAudio_Init() {
	...
	if ( dwReturn = mciSendCommand( 0, MCI_OPEN, MCI_OPEN_TYPE | MCI_OPEN_SHAREABLE, ( DWORD )( LPVOID )&mciOpenParms ) ) {
		common->Printf( "CDAudio_Init: MCI_OPEN failed (%i)\n", dwReturn );
		return -1;
	}
	...
	if ( dwReturn = mciSendCommand( wDeviceID, MCI_SET, MCI_SET_TIME_FORMAT, ( DWORD )( LPVOID )&mciSetParms ) ) {
		common->Printf( "MCI_SET_TIME_FORMAT failed (%i)\n", dwReturn );
...
void CDAudio_Shutdown() {
	...
	if ( mciSendCommand( wDeviceID, MCI_CLOSE, MCI_WAIT, ( DWORD )NULL ) ) {
		common->DPrintf( "CDAudio_Shutdown: MCI_CLOSE failed\n" );
	}
}
