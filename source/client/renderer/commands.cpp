...
void R_InitCommandBuffers() {
	...
	if ( ( GGameType & GAME_Tech3 ) && r_smp->integer ) {
		common->Printf( "Trying SMP acceleration...\n" );
...
static void R_PerformanceCounters() {
	...
	if ( GGameType & GAME_Quake2 ) {
		common->Printf( "%4i wpoly %4i epoly %i tex %i lmaps\n",
	...
	if ( r_speeds->integer == 1 ) {
		common->Printf( "%i/%i shaders/surfs %i leafs %i verts %i/%i tris %.2f mtex %.2f dc\n",
		...
		common->Printf( "(md3) %i sin %i sclip  %i sout %i bin %i bclip %i bout\n",
...
void* R_GetCommandBuffer( int bytes ) {
	...
	if ( cmdList->used + bytes + sizeof ( swapBuffersCommand_t ) + 4 > MAX_RENDER_COMMANDS ) {
		if ( bytes > MAX_RENDER_COMMANDS - ( int )sizeof ( swapBuffersCommand_t ) - 4 ) {
			common->FatalError( "R_GetCommandBuffer: bad size %i", bytes );
		}
...
void R_BeginFrame( stereoFrame_t stereoFrame ) {
	...
	if ( !r_ignoreGLErrors->integer ) {
		...
		int err = qglGetError();
		if ( err != GL_NO_ERROR ) {
			common->FatalError( "RE_BeginFrame() - glGetError() failed (0x%x)!\n", err );
		}
	}
