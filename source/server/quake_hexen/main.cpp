...
static void SVCQHW_Log( const netadr_t& net_from ) {
	...
	if ( seq == svs.qh_logsequence - 1 || !svqhw_fraglogfile ) {// they allready have this data, or we aren't logging frags
		data[ 0 ] = A2A_NACK;
...
static void SVCQHW_DirectConnect( const netadr_t& net_from ) {
	...
	if ( s[ 0 ] && String::Cmp( s, "0" ) ) {
		if ( svqhw_spectator_password->string[ 0 ] &&
			 ...
			 String::Cmp( svqhw_spectator_password->string, s ) ) {	// failed
