...
static idEntity3* SVQ3_EntityForGentity(const q3sharedEntity_t* gEnt)
{
	if (!gEnt || gEnt->s.number < 0 || gEnt->s.number >= MAX_GENTITIES_Q3)
	{
		common->Error("SVQ3_SvEntityForGentity: bad gEnt");
...
{
	VM_Call(gvm, Q3GAME_INIT, serverTime, randomSeed, restart);
}

void SVQ3_GameShutdown(bool restart)
{
	VM_Call(gvm, Q3GAME_SHUTDOWN, restart);
}
...
static void SVQ3_LinkEntity(q3sharedEntity_t* gEnt)
{
